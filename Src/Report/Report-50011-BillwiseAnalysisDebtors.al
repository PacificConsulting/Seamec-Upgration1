report 50011 "Billwise Analysis (Debtors)"
{
    DefaultLayout = RDLC;
            ...
            dataitem(CurrencyLoop; Integer)
            {
                ...
                dataitem(TempCustLedgEntryLoop; Integer)
                {
                    ...
                    begin
                        ...
                        recSalesInvHeader.SETRANGE(recSalesInvHeader."Sell-to Customer No.", CustLedgEntryEndingDate."Customer No.");
                        recSalesInvHeader.SETRANGE(recSalesInvHeader."No.", CustLedgEntryEndingDate."Document No.");
                        IF recSalesInvHeader.FIND('-') THEN BEGIN
                            dataNarration := ''; //recSalesInvHeader."Invoice Description"; PCPL-064
                        END;
                        //001--
                        CASE AgingBy OF
                            AgingBy::"Due Date":
                        ...
                        //002
                        IF ((TotalCustLedgEntry[1]."Amount (LCY)") = 0) OR (TotalCustLedgEntry[1].Amount = 0) THEN
                            CurrReport.SKIP;
                        //002
                        //001++
                        IF PrintDetails THEN
                            dataExRate := CustLedgEntryEndingDate."Amount (LCY)" / CustLedgEntryEndingDate.Amount
                        ELSE
                            dataExRate := TotalCustLedgEntry[1]."Amount (LCY)" / TotalCustLedgEntry[1].Amount;
                        //001--
                    end;
    ...
    var
        ...
        dataCompName: Text[30];
        Text011: Label 'Not Yet Due';
